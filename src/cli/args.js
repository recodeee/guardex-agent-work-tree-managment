...
function normalizeManagedForcePath(rawPath) {
  ...
  const normalized = require('node:path').posix.normalize(rawPath.replace(/\\/g, '/'));
  if (
    !normalized ||
    normalized === '.' ||
    normalized.startsWith('../') ||
    require('node:path').posix.isAbsolute(normalized)
  ) {
    return null;
  }
...
function parseCommonArgs(rawArgs, defaults, options = {}) {
  const parsed = { ...defaults };
  const supportsForce = Object.prototype.hasOwnProperty.call(parsed, 'force');
  if (supportsForce && !Array.isArray(parsed.forceManagedPaths)) {
    parsed.forceManagedPaths = [];
  }

  for (let index = 0; index < rawArgs.length; index += 1) {
    const arg = rawArgs[index];
    if (arg === '--target' || arg === '-t') {
      parsed.target = requireValue(rawArgs, index, '--target');
      index += 1;
      continue;
    }
    if (arg === '--dry-run') {
    ...
    if (arg === '--force') {
      ...
      parsed.force = true;
      const collected = collectForceManagedPaths(rawArgs, index);
      if (collected.forceManagedPaths.length > 0) {
        parsed.forceManagedPaths = Array.from(
          new Set([...(parsed.forceManagedPaths || []), ...collected.forceManagedPaths]),
        );
      }
    ...
    if (arg === '--yes-global-install') {
      parsed.yesGlobalInstall = true;
      continue;
    }
    if (arg === '--no-global-install') {
      parsed.noGlobalInstall = true;
      continue;
    }
    if (arg === '--no-gitignore') {
      parsed.skipGitignore = true;
      continue;
    }
    if (arg === '--allow-protected-base-write') {
      parsed.allowProtectedBaseWrite = true;
      continue;
    }
    if (Object.prototype.hasOwnProperty.call(parsed, 'waitForMerge') && arg === '--wait-for-merge') {
      parsed.waitForMerge = true;
      continue;
    }
    if (Object.prototype.hasOwnProperty.call(parsed, 'waitForMerge') && arg === '--no-wait-for-merge') {
      parsed.waitForMerge = false;
      continue;
    }
...
function parseRepoTraversalArgs(rawArgs, defaults, options = {}) {
  const nestedRepoDefaultMaxDepth = options.nestedRepoDefaultMaxDepth ?? 6;
  const traversalDefaults = {
    ...defaults,
    recursive: true,
    nestedMaxDepth: nestedRepoDefaultMaxDepth,
    nestedSkipDirs: [],
  ...
  return parseCommonArgs(forwardedArgs, traversalDefaults, options);
}

function parseSetupArgs(rawArgs, defaults, options = {}) {
  const setupDefaults = {
    ...defaults,
  ...
  return parseRepoTraversalArgs(forwardedArgs, setupDefaults, options);
}
...
function parseDoctorArgs(rawArgs, options = {}) {
  ...
  return parseRepoTraversalArgs(forwardedArgs, doctorDefaults, options);
}
