...
const runtimeModule = require('../core/runtime');
const gitModule = require('../git');
const scaffoldModule = require('../scaffold');
...
const cliArgsModule = require('./args');
const cliDispatchModule = require('./dispatch');
...
function runtimeVersion() {
  return outputModule.runtimeVersion();
}

function supportsAnsiColors() {
  return outputModule.supportsAnsiColors();
}

function colorize(text, colorCode) {
  return outputModule.colorize(text, colorCode);
}

function doctorOutputColorCode(status) {
  return outputModule.doctorOutputColorCode(status);
}

function colorizeDoctorOutput(text, status) {
  return outputModule.colorizeDoctorOutput(text, status);
}

function detectAutoFinishDetailStatus(detail) {
  return outputModule.detectAutoFinishDetailStatus(detail);
}

function detectAutoFinishSummaryStatus(summary) {
  return outputModule.detectAutoFinishSummaryStatus(summary);
}

function statusDot(status) {
  return outputModule.statusDot(status);
}

function commandCatalogLines(indent = '  ') {
  return outputModule.commandCatalogLines(indent);
}

function agentBotCatalogLines(indent = '  ') {
  return outputModule.agentBotCatalogLines(indent);
}

function repoToggleLines(indent = '  ') {
  return outputModule.repoToggleLines(indent);
}

function printToolLogsSummary() {
  return outputModule.printToolLogsSummary();
}

function usage(options = {}) {
  return outputModule.usage(options);
}

function run(cmd, args, options = {}) {
  return runtimeModule.run(cmd, args, options);
}

function extractTargetedArgs(rawArgs, defaultTarget = process.cwd()) {
  return runtimeModule.extractTargetedArgs(rawArgs, defaultTarget);
}

function packageAssetEnv(extraEnv = {}) {
  return runtimeModule.packageAssetEnv(extraEnv);
}

function packageAssetPath(assetKey) {
  return runtimeModule.packageAssetPath(assetKey);
}

function runPackageAsset(assetKey, rawArgs, options = {}) {
  return runtimeModule.runPackageAsset(assetKey, rawArgs, options);
}

function repoLocalLegacyScriptPath(repoRoot, relativePath) {
  return runtimeModule.repoLocalLegacyScriptPath(repoRoot, relativePath);
}

function runReviewBotCommand(repoRoot, rawArgs, options = {}) {
  return runtimeModule.runReviewBotCommand(repoRoot, rawArgs, options);
}

function invokePackageAsset(assetKey, rawArgs, options = {}) {
  return runtimeModule.invokePackageAsset(assetKey, rawArgs, options);
}

function formatElapsedDuration(ms) {
  return outputModule.formatElapsedDuration(ms);
}

function truncateMiddle(value, maxLength) {
  return outputModule.truncateMiddle(value, maxLength);
}

function truncateTail(value, maxLength) {
  return outputModule.truncateTail(value, maxLength);
}

function compactAutoFinishPathSegments(message) {
  return outputModule.compactAutoFinishPathSegments(message);
}

function detectRecoverableAutoFinishConflict(message) {
  return outputModule.detectRecoverableAutoFinishConflict(message);
}

function summarizeAutoFinishDetail(detail) {
  return outputModule.summarizeAutoFinishDetail(detail);
}

function printAutoFinishSummary(summary, options = {}) {
  return outputModule.printAutoFinishSummary(summary, options);
}

function gitRun(repoRoot, args, { allowFailure = false } = {}) {
  return gitModule.gitRun(repoRoot, args, { allowFailure });
}

function resolveRepoRoot(targetPath) {
  return gitModule.resolveRepoRoot(targetPath);
}

function isGitRepo(targetPath) {
  return gitModule.isGitRepo(targetPath);
}

const NESTED_REPO_DEFAULT_MAX_DEPTH = gitModule.DEFAULT_NESTED_REPO_MAX_DEPTH;
function discoverNestedGitRepos(rootPath, opts = {}) {
  return gitModule.discoverNestedGitRepos(rootPath, opts);
}

function toDestinationPath(relativeTemplatePath) {
  return scaffoldModule.toDestinationPath(relativeTemplatePath);
}

function ensureParentDir(repoRoot, filePath, dryRun) {
  return scaffoldModule.ensureParentDir(repoRoot, filePath, dryRun);
}

function ensureExecutable(destinationPath, relativePath, dryRun) {
  return scaffoldModule.ensureExecutable(destinationPath, relativePath, dryRun);
}

function isCriticalGuardrailPath(relativePath) {
  return scaffoldModule.isCriticalGuardrailPath(relativePath);
}

function shellSingleQuote(value) {
  return scaffoldModule.shellSingleQuote(value);
}

function renderShellDispatchShim(commandParts) {
  return scaffoldModule.renderShellDispatchShim(commandParts);
}

function renderPythonDispatchShim(commandParts) {
  return scaffoldModule.renderPythonDispatchShim(commandParts);
}

function managedForceConflictMessage(relativePath) {
  return scaffoldModule.managedForceConflictMessage(relativePath);
}
...
function configureHooks(repoRoot, dryRun) {
  return hooksModule.configureHooks(repoRoot, dryRun);
}
...
function parseCommonArgs(rawArgs, defaults) {
  return cliArgsModule.parseCommonArgs(rawArgs, defaults);
}

function parseRepoTraversalArgs(rawArgs, defaults) {
  return cliArgsModule.parseRepoTraversalArgs(rawArgs, defaults, {
    nestedRepoDefaultMaxDepth: NESTED_REPO_DEFAULT_MAX_DEPTH,
  });
}

function parseSetupArgs(rawArgs, defaults) {
  return cliArgsModule.parseSetupArgs(rawArgs, defaults, {
    nestedRepoDefaultMaxDepth: NESTED_REPO_DEFAULT_MAX_DEPTH,
  });
}
...
function parseTargetFlag(rawArgs, defaultTarget = process.cwd()) {
  return cliArgsModule.parseTargetFlag(rawArgs, defaultTarget);
}

function parseReviewArgs(rawArgs) {
  return cliArgsModule.parseReviewArgs(rawArgs);
}

function parseAgentsArgs(rawArgs) {
  return cliArgsModule.parseAgentsArgs(rawArgs);
}

function parseReportArgs(rawArgs) {
  return cliArgsModule.parseReportArgs(rawArgs);
}
...
function parseSyncArgs(rawArgs) {
  return cliArgsModule.parseSyncArgs(rawArgs);
}

function parseCleanupArgs(rawArgs) {
  return cliArgsModule.parseCleanupArgs(rawArgs);
}

function parseMergeArgs(rawArgs) {
  return cliArgsModule.parseMergeArgs(rawArgs);
}

function parseFinishArgs(rawArgs, defaults = {}) {
  return cliArgsModule.parseFinishArgs(rawArgs, defaults);
}
...
function printOperations(title, payload, dryRun = false) {
  return scaffoldModule.printOperations(title, payload, dryRun);
}
...
function printStandaloneOperations(title, rootLabel, operations, dryRun = false) {
  return scaffoldModule.printStandaloneOperations(title, rootLabel, operations, dryRun);
}
...
function runFromBin(argv = process.argv.slice(2)) {
  try {
    main(argv);
  } catch (error) {
